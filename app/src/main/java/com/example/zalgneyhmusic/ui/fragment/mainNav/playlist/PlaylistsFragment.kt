...
class PlaylistsFragment : BaseFragment() {
    ...
    private fun observeCreateState() {
        viewModel.createPlaylistState.observe(viewLifecycleOwner) { resource ->
            if (resource is Resource.Success) {
                ...
                Toast.makeText(context, getString(R.string.error, resource.exception.message), Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun showCreatePlaylistDialog() {
        ...
        currentDialogBinding = dialogBinding // Used by image picker launcher
        ...
        // Image selection
        dialogBinding.imageView.setOnClickListener { pickImageLauncher.launch("image/*") }
        ...
        dialogBinding.btnCreate.setOnClickListener {
            ...
            if (name.isNotEmpty()) {
                val imageFile = selectedImageUri?.let { uri -> StorageHelper.uriToFile(requireContext(), uri) }
        ...
        // Clear binding when dialog dismissed
    ...
    private fun setupRecyclerView() {
        playlistAdapter = PlaylistAdapter(
            onPlaylistClick = { playlist -> openPlaylistDetail(playlist.id) },
    ...
    /** Observe playlists LiveData and update UI. */
        ...
        viewModel.userPlaylists.observe(viewLifecycleOwner) { resource ->
            when (resource) {
                ...
                is Resource.Failure -> {
                    ...
                    binding.txtError.text = when (msg) {
                        "error_sign_in_required" -> getString(R.string.error_sign_in_required)
                        "playlist_deleted" -> getString(R.string.playlist_deleted)
                        "playlist_updated_success" -> getString(R.string.playlist_updated_success)
                        else -> msg ?: getString(R.string.unknown_error)
                    }
                }
            }
        }
